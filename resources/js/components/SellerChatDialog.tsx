...
export const SellerChatDialog: React.FC<SellerChatDialogProps> = ({
    ...
    const [selectedUserId, setSelectedUserId] = useState<number | null>(preferredUserId ?? null);
    const [selectedUser, setSelectedUser] = useState<DirectMessageParticipant | null>(buildFallbackParticipant(preferredUserId, preferredUserName));
    ...
    useEffect(() => {
        ...
        setSelectedUser(buildFallbackParticipant(preferredUserId, preferredUserName));
        setDraft('');
        setSelectedFiles([]);
    ...
    useEffect(() => {
        ...
        const loadThread = async (isSilent = false) => {
            ...
            try {
                const data = await directMessageService.getThread(selectedUserId);
                if (!isActive) {
                    return;
                }

                setSelectedUser(data.user ?? buildFallbackParticipant(selectedUserId, preferredUserName));
    ...
    const handleSendMessage = async () => {
        ...
        try {
            const createdMessage = await directMessageService.sendMessage(selectedUserId, {
                message: draft.trim(),
            ...
            setThreads((prev) => {
                const nextThread: DirectMessageThread = {
                    user: selectedUser,
                    latest_message: createdMessage.message.trim() || (createdMessage.attachments?.length === 1 ? 'Sent an attachment' : `Sent ${createdMessage.attachments?.length ?? 0} attachments`),
                    latest_message_at: createdMessage.created_at,
                    unread_count: 0,
                };
                const others = prev.filter((thread) => thread.user?.id !== selectedUserId);
        ...
        try {
            ...
            mediaRecorder.addEventListener('stop', () => {
                ...
                if (blob.size > 0) {
                    ...
                    const voiceFile = new File([blob], `voice-clip-${Date.now()}.${extension}`, { type: blob.type || 'audio/webm' });
                ...
                <div className="seller-chat-dialog-content">
                    <aside className="seller-chat-left-panel">
                        <div className="seller-chat-tools">
                            ...
                            />
                            <select className="seller-chat-filter" value={threadFilter} onChange={(event) => setThreadFilter(event.target.value as 'all' | 'unread')}>
                        ...
                        <div className="seller-chat-thread-list">
                            {threadsLoading && filteredThreads.length === 0 && <p className="seller-chat-thread-empty">Loading conversations...</p>}
                    ...
                    {selectedUserId ? (
                        <main className="seller-chat-main-pane">
                            ...
                            <div className="seller-chat-message-list">
                                {messagesLoading && messages.length === 0 && <p className="seller-chat-message-empty">Loading conversation...</p>}
                                {!messagesLoading && messages.length === 0 && <p className="seller-chat-message-empty">Start the conversation with this seller.</p>}

                                {messages.map((message) => {
                                    const isOwnMessage = message.sender_id === authUser.id;
                                    return (
                                        <article key={message.id} className={`seller-chat-message-bubble ${isOwnMessage ? 'is-own' : 'is-other'}`}>
                                            {message.message.trim() && <p className="seller-chat-message-text">{message.message}</p>}
                                                    ...
                                                    {message.attachments.map((attachment) => (
                                                        ...
                                                        >
                                                            {isImageAttachment(attachment) ? (
                                                                ...
                                                                <audio className="seller-chat-attachment-audio" controls src={attachment.url}>
                                                                    Your browser does not support the audio element.
                                                                </audio>
                                                            ) : (
                                                                <div className="seller-chat-attachment-file">
                                                                    <span className="seller-chat-attachment-file-name">{attachment.file_name}</span>
                                                                    <span className="seller-chat-attachment-file-meta">{formatFileSize(attachment.file_size)}</span>
                                                                </div>
                                                            )}
                                                        </a>
                                                    ))}
                                            ...
                                            <span className="seller-chat-message-time">{formatDate(message.created_at)}</span>
                                        </article>
                                    );
                                })}
                            </div>
                            ...
                            <div className="seller-chat-compose">
                                <div className="seller-chat-compose-actions">
                                    <label className="seller-chat-icon-btn" aria-label="Attach files" title="Attach files">
                                        <input
                                        ...
                                        />
                                        <svg viewBox="0 0 24 24" role="img" focusable="false">
                                            <path fill="currentColor" d="M16.5 6.5v8.75a4.25 4.25 0 1 1-8.5 0V5.75a2.75 2.75 0 0 1 5.5 0V14a1.25 1.25 0 0 1-2.5 0V7h-1.5v7a2.75 2.75 0 0 0 5.5 0V5.75a4.25 4.25 0 1 0-8.5 0v9.5a5.75 5.75 0 1 0 11.5 0V6.5Z" />
                                        </svg>
                                    </label>
                                    <button
                                        type="button"
                                    ...
                                    >
                                        <svg viewBox="0 0 24 24" role="img" focusable="false">
                                            <path fill="currentColor" d="M12 22a10 10 0 1 1 10-10 10 10 0 0 1-10 10Zm0-18.5A8.5 8.5 0 1 0 20.5 12 8.51 8.51 0 0 0 12 3.5Zm-3 6.25A1.25 1.25 0 1 1 10.25 8.5 1.25 1.25 0 0 1 9 9.75Zm6 0A1.25 1.25 0 1 1 16.25 8.5 1.25 1.25 0 0 1 15 9.75Zm-3 7a5.56 5.56 0 0 1-4.22-1.93l1.14-.97a4.05 4.05 0 0 0 6.16 0l1.14.97A5.56 5.56 0 0 1 12 16.75Z" />
                                        </svg>
                                    </button>
                                    <button
                                        type="button"
                                        ...
                                        aria-label={isRecording ? 'Stop voice recording' : 'Record voice clip'}
                                        title={isRecording ? 'Stop voice recording' : 'Record voice clip'}
                                    >
                                        <svg viewBox="0 0 24 24" role="img" focusable="false">
                                            <path fill="currentColor" d="M12 15.75A3.75 3.75 0 0 0 15.75 12V6a3.75 3.75 0 0 0-7.5 0v6A3.75 3.75 0 0 0 12 15.75Zm5-3.75a.75.75 0 0 1 1.5 0A6.51 6.51 0 0 1 12.75 18.44V21h2.5a.75.75 0 0 1 0 1.5h-6.5a.75.75 0 0 1 0-1.5h2.5v-2.56A6.51 6.51 0 0 1 5.5 12a.75.75 0 0 1 1.5 0 5 5 0 1 0 10 0Z" />
                                        </svg>
                                    </button>
                                    {isEmojiPickerOpen && (
                                    ...
                                    {isRecording && <div className="seller-chat-recording-indicator" aria-live="polite">●</div>}
                                    {selectedFiles.length > 0 && (
                                        <div className="seller-chat-selected-files">
                                            {selectedFiles.map((file) => (
                                                <div key={`${file.name}-${file.size}`} className="seller-chat-selected-file-chip">
                                                    <span>{file.name}</span>
                                                    <button type="button" onClick={() => handleRemoveSelectedFile(file.name)} aria-label={`Remove ${file.name}`}>
                                                        ×
                                                    </button>
                                                </div>
                                            ))}
                                ...
                                <div className="seller-chat-compose-row">
                                    <p className="seller-chat-compose-note">Messages and attachments are sent directly to this seller.</p>
                                    <button type="button" className="seller-chat-send-btn" onClick={handleSendMessage} disabled={sending || (!draft.trim() && selectedFiles.length === 0)} aria-label="Send message" title="Send message">
                                        <svg viewBox="0 0 24 24" role="img" focusable="false">
                                            <path fill="currentColor" d="M3.41 20.59a.75.75 0 0 1 .1-.93L18.12 5.05 4.57 10.13a.75.75 0 0 1-.52-1.4l15.75-5.91a.75.75 0 0 1 .97.97L14.86 19.54a.75.75 0 0 1-1.4-.52l5.08-13.55L4.34 20.08a.75.75 0 0 1-.93.1Z" />
                                        </svg>
                                    </button>
                                </div>
                            </div>
