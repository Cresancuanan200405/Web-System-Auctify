...
export const BagPage: React.FC<BagPageProps> = ({ onNavigateHome, onNavigateToAuction }) => {
    const { authUser } = useAuth();
    const { savedCards, mainCardId, setMainCardId, updateCard, getCardDisplayName } = useCards();
    const { wonAuctions, isLoadingWonAuctions } = useWonAuctions();
    const { orders, addOrder } = useOrderHistory();
    const [addresses, setAddresses] = useState<Address[]>([]);
    const [addressLoading, setAddressLoading] = useState(false);
    const [checkoutItem, setCheckoutItem] = useState<BagAuctionItem | null>(null);
    ...
    const activeBagItems = useMemo(
        () => wonAuctions.filter((item) => !orders.some((order) => order.auction_id === item.id)),
        [orders, wonAuctions],
    );
            ...
            <main className="content">
                <div className="bag-page-container">
                    <div className="bag-empty-state">
                        ...
                        <p className="bag-subtitle">Checking the auctions you have won.</p>
                    </div>
                </div>
            </main>
        );
            ...
            <main className="content">
                <div className="bag-page-container bag-page-filled">
                    <div className="bag-page-head">
                        <div>
                            <h1 className="bag-page-title">Your Won Auctions</h1>
                            <p className="bag-page-subtitle">Every auction you win is added here automatically.</p>
                        </div>
                        <span className="bag-page-count">{activeBagItems.length} item(s)</span>
                    </div>
                                    ...
                                    <div className="bag-item-media-wrap">
                                        {mediaUrl ? (
                                            firstMedia?.media_type === 'video' ? (
                                                <video className="bag-item-media" src={mediaUrl} muted preload="metadata" />
                                            ) : (
                                                <img className="bag-item-media" src={mediaUrl} alt={item.title} />
                                            )
                                        ) : (
                                            <div className="bag-item-media bag-item-media-empty">No Media</div>
                                        )}
                                    </div>
                                    <div className="bag-item-content">
                                        <p className="bag-item-badge">Won</p>
                                        <p className="bag-item-title">{item.title}</p>
                                        <p className="bag-item-meta">Seller: {sellerName}</p>
                                        <p className="bag-item-meta">Winning bid: {formatCurrency(Number(item.winning_bid_amount ?? 0))}</p>
                                        <div className="bag-item-actions">
                                            <button type="button" className="bag-item-open-btn" onClick={(event) => {
                                                event.stopPropagation();
                                                onNavigateToAuction(item.id);
                                            }}>
                                                View Auction
                                            </button>
                                            <button type="button" className="bag-item-checkout-btn" onClick={(event) => {
                                                event.stopPropagation();
                ...
                {checkoutItem && canRenderPortal && createPortal(
                    <div className="bag-checkout-overlay" onClick={() => !isCheckingOut && setCheckoutItem(null)}>
                        <div className="bag-checkout-dialog" onClick={(event) => event.stopPropagation()}>
                            <div className="bag-checkout-head">
                                ...
                                <button type="button" className="bag-checkout-close" onClick={() => setCheckoutItem(null)} disabled={isCheckingOut}>
                                    ×
                                </button>
                            </div>
                            <div className="bag-checkout-body">
                                <div className="bag-checkout-summary">
                                    <p className="bag-checkout-item-title">{checkoutItem.title}</p>
                                    <p className="bag-checkout-item-meta">Winning total: {formatCurrency(Number(checkoutItem.winning_bid_amount ?? 0))}</p>
                                </div>
                                <label className="bag-checkout-label">
                                    Delivery address
                                    <select value={selectedAddressId} onChange={(event) => setSelectedAddressId(event.target.value)} disabled={addressLoading || isCheckingOut}>
                                ...
                                <div className="checkout-deduction-box" aria-live="polite">
                                    <div className="checkout-deduction-row">
                                        ...
                                        <strong>{formatCurrency(checkoutAmount)}</strong>
                                    </div>
                                    <div className="checkout-deduction-row">
                                        <span>Card balance after payment</span>
                                        <strong>{remainingBalance !== null ? formatCurrency(Math.max(0, remainingBalance)) : '--'}</strong>
                                    </div>
                                </div>
                            ...
                            <div className="bag-checkout-actions">
                                <button type="button" className="bag-checkout-cancel" onClick={() => setCheckoutItem(null)} disabled={isCheckingOut}>
                                    Cancel
                                </button>
                                <button type="button" className="bag-checkout-submit" onClick={handleConfirmCheckout} disabled={isCheckingOut || !addresses.length || !savedCards.length}>
