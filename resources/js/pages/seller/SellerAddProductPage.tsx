...
export const SellerAddProductPage: React.FC<SellerAddProductPageProps> = ({
    ...
    const [endTimeMode, setEndTimeMode] = useState<
        'days' | 'hours' | 'minutes'
    >('days');
    ...
    const handleMediaChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setMediaEntries((prevEntries) => {
            ...
            if (validFiles.length !== selectedFiles.length) {
                setMediaError(
                    'Only image and video files are allowed, with a maximum of 10 entries.',
                );
            }
    ...
    const handleRemoveMedia = (entryId: string) => {
        setMediaEntries((prevEntries) => {
            const entryToRemove = prevEntries.find(
    ...
    const buildBidSchedule = useCallback(() => {
        ...
        if (startMode === 'scheduled') {
            ...
            if (Number.isNaN(scheduledDate.getTime()) || scheduledDate <= now) {
                return {
                    error: 'Scheduled start time must be a valid future date and time.',
                };
            }
        ...
        const endsAt = new Date(
            startsAt.getTime() + getDurationMs(endTimeMode, durationValue),
        );
        if (endsAt <= startsAt) {
            return {
                error: 'End date and time must be after the bidding start time.',
            };
        }
    ...
    const handleSaveProduct = async () => {
        ...
        const mediaEntriesOrderedForDisplay = [...mediaEntries].sort(
            (left, right) => {
            ...
            <div className="seller-add-header">
                <div>
                    ...
                    <p className="seller-add-subtitle">
                        Create a polished listing with strong visuals, clear
                        pricing, and the right auction timing.
                    </p>
                </div>
            ...
            <div className="seller-add-layout">
                <div className="seller-add-main">
                        ...
                        <div className="seller-add-grid">
                            ...
                            <div className="seller-input-wrap">
                                ...
                                <input
                                    ...
                                    value={maxIncrement}
                                    onChange={(event) =>
                                        setMaxIncrement(event.target.value)
                                    }
                                />
                            </div>
                            <div className="seller-input-wrap seller-add-grid-full">
                    ...
                    <div className="seller-add-card seller-add-section-card">
                        <div className="seller-add-section-head">
                            <h3>Auction Setup</h3>
                            <p>
                                Configure when bidding begins and how long the
                                auction runs.
                            </p>
                        </div>
                        <div className="seller-add-grid">
                        ...
                        <div className="seller-add-auction-note">
                            ...
                            <p className="seller-field-help seller-schedule-preview">
                                {bidSchedulePreview}
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="seller-add-card seller-add-section-card">
                        <div className="seller-add-section-head">
                            <h3>Media Gallery</h3>
                            <p>
                                Upload up to 10 images or videos. Strong cover
                                media increases conversions.
                            </p>
                        </div>
                        <div className="seller-input-wrap seller-add-grid-full">
                            <label className="seller-add-label">
                                Product Media (Images or Videos, up to 10)
                            </label>
                            <input
                                type="file"
                            ...
                            />
                            <p className="seller-media-help">
                                {mediaEntries.length}/10 uploaded
                            </p>
                            {mediaError && (
                                <p className="seller-media-error">
                                    {mediaError}
                                </p>
                            )}
                ...
                <aside className="seller-add-side">
                    <div className="seller-add-side-card">
                        ...
                        <p className="seller-add-side-title">
                            {productName.trim() ||
                        ...
                        <div className="seller-add-side-meta">
                            ...
                            <p>
                                <span>Media</span>
                                <strong>{mediaEntries.length}/10</strong>
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="seller-add-side-card seller-add-side-note">
                        ...
                        <ul>
                            <li className={productName.trim() ? 'done' : ''}>
                                Clear product title
                            </li>
                            <li className={category ? 'done' : ''}>
                                Correct category selected
                            </li>
                            <li className={subcategory ? 'done' : ''}>
                                Correct subcategory selected
                            </li>
                            <li className={description.trim() ? 'done' : ''}>
                                Detailed description
                            </li>
                            <li
                                className={
                                    mediaEntries.length > 0 ? 'done' : ''
                                }
                            >
