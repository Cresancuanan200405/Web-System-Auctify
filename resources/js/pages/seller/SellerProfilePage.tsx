...
interface SellerProfilePageProps {
    onNavigateSellerDashboard: (
        section?: 'products' | 'shipping' | 'orders',
    ) => void;
...
export const SellerProfilePage: React.FC<SellerProfilePageProps> = ({
    ...
    useEffect(() => {
        ...
        const load = async () => {
            setLoading(true);
            try {
                const [registrationResponse, productsResponse] =
                    await Promise.all([
    ...
    const shopName =
        registration?.shop_name?.trim() || authUser?.name || 'Your shop';
    const sellerTypeLabel =
        registration?.seller_type === 'corp'
            ? 'Corporation'
    ...
    const openProducts = products.filter(
        (product) => getAuctionDisplayStatus(product) === 'open',
    );
    const closedProducts = products.filter(
        (product) => getAuctionDisplayStatus(product) === 'closed',
    );
    const scheduledProducts = products.filter(
        (product) => getAuctionDisplayStatus(product) === 'scheduled',
    );
    ...
    const featuredProducts = products
        ...
        .sort((left, right) => {
            return (
                new Date(right.created_at || right.ends_at).getTime() -
                new Date(left.created_at || left.ends_at).getTime()
            );
        })
        .slice(0, 4);
    ...
    const sellerTags = useMemo(() => {
        return [
            ...
            sellerTypeLabel,
            registration?.vat_status === 'vat'
                ? 'VAT Registered'
                : registration?.vat_status === 'non-vat'
                  ? 'Non-VAT'
                  : null,
            registration?.contact_email?.trim()
                ? 'Email Verified for Operations'
                : null,
    ...
    return (
        <section className="seller-profile-page">
            <div className="seller-profile-breadcrumb">
                <button
                    type="button"
                    onClick={() => onNavigateSellerDashboard('products')}
                >
                    Seller Dashboard
                </button>
                <span>›</span>
                    ...
                    <div>
                        ...
                        <div className="seller-profile-title-row">
                            ...
                            <div className="seller-profile-status">
                                <span
                                    className="seller-profile-status-dot"
                                    aria-hidden="true"
                                ></span>
                        ...
                        <p className="seller-profile-subtitle">
                            Manage the storefront details buyers trust most:
                            business identity, fulfillment contacts, listing
                            activity, and order performance in one
                            seller-focused space.
                        </p>
                        <div className="seller-profile-tags">
                            {sellerTags.map((tag) => (
                                <span key={tag} className="seller-profile-tag">
                                    {tag}
                                </span>
                            ))}
                        </div>
                        <div className="seller-profile-actions">
                            ...
                            <button
                                type="button"
                                className="seller-profile-cta-secondary"
                                onClick={() =>
                                    onNavigateSellerDashboard('shipping')
                                }
                            >
                                Edit Seller Info
                            </button>
                            <button
                                ...
                                className="seller-profile-cta-secondary"
                                onClick={() =>
                                    onNavigateSellerDashboard('orders')
                                }
                            >
                                Review Orders
                            </button>
            ...
            <div className="seller-profile-layout">
                    ...
                    <article className="seller-profile-panel">
                        <div className="seller-profile-panel-head">
                            <div>
                                ...
                                <p>
                                    Core seller information shown to operations
                                    and used across your storefront setup.
                                </p>
                            </div>
                        </div>
                        <div className="seller-profile-info-grid">
                            ...
                            <div className="seller-profile-info-card">
                                ...
                                <p>
                                    {registration?.pickup_address_summary ||
                                        registration?.registered_address ||
                                        'Not available'}
                                </p>
                            </div>
                        </div>
                    ...
                    <article className="seller-profile-panel">
                        <div className="seller-profile-panel-head">
                            <div>
                                ...
                                <p>
                                    Key marketplace metrics buyers and internal
                                    operations care about most.
                                </p>
                            </div>
                        </div>
                        <div className="seller-profile-highlight-grid">
                            <div className="seller-profile-highlight">
                                ...
                                <p>
                                    All active, scheduled, and archived listings
                                </p>
                            </div>
                            <div className="seller-profile-highlight">
                    ...
                    <article className="seller-profile-panel">
                        <div className="seller-profile-panel-head">
                            <div>
                                ...
                                <p>
                                    The latest products published from this
                                    seller account.
                                </p>
                            </div>
                        </div>
                        {loading ? (
                            ...
                            <div className="seller-profile-empty">
                                No products published yet. Add your first
                                listing from Seller Dashboard.
                            </div>
                        ) : (
                            <div className="seller-profile-product-list">
                                {featuredProducts.map((product) => {
                                    ...
                                    const status =
                                        getAuctionDisplayStatus(product);
                                    return (
                                        <div
                                            key={product.id}
                                            className="seller-profile-product-item"
                                        >
                                            {media ? (
                                                <img
                                                    className="seller-profile-product-thumb"
                                                    src={resolveMediaUrl(
                                                        media.url,
                                                    )}
                                                    alt={product.title}
                                                ...
                                                <div className="seller-profile-product-thumb-empty">
                                                    No image
                                                </div>
                                            )}
                                            ...
                                            <span
                                                className={`seller-profile-product-status seller-profile-product-status-${status}`}
                                            >
                                                {status}
                                            </span>
                                        </div>
                ...
                <aside className="seller-profile-side">
                        ...
                        <div className="seller-profile-side-list">
                            ...
                            <div className="seller-profile-side-list-item">
                                <div>
                                    ...
                                    <strong>
                                        {registration?.primary_document_type ||
                                            'Not submitted'}
                                    </strong>
                                </div>
                            </div>
                            <div className="seller-profile-side-list-item">
                                <div>
                                    ...
                                    <strong>
                                        {registration?.government_id_type ||
                                            'Not submitted'}
                                    </strong>
                                </div>
                            </div>
                    ...
                    <article className="seller-profile-side-card">
                        ...
                        <p>
                            Operational contact points used for shipping
                            coordination and buyer confidence.
                        </p>
                        <div className="seller-profile-side-list">
                            <div className="seller-profile-side-list-item">
                                <div>
