...
export function useCards() {
    ...
    const [savedCards, setSavedCards] = useLocalStorage<Card[]>(
        scopedSavedCardsKey,
        [],
    );
    const [mainCardId, setMainCardIdState] = useLocalStorage<number | null>(
        scopedMainCardKey,
        null,
    );
    ...
    useEffect(() => {
        ...
        const scopedMainCardRaw =
            window.localStorage.getItem(scopedMainCardKey);
